 ...
 *
 * FIXME: should be merged with pt_aes{192,256}_add_round_key()
 */
...
/**
 * Expand 24 byte input key into an output array of 52 unsigned 32-bit
 * integers of key schedule data.
 *
 ...
 *
 * FIXME: should be merged with pt_aes{128,256}_add_round_key()
 *
 * Parameters:
...
/**
 * Expand 32 byte input key into an output array of 60 unsigned 32-bit
 * integers of key schedule data.
 *
 * The output key schedule data is used by pt_aes256_enc() and
 * pt_aes256_dec().
 *
 * Parameters:
 * - dst: Destination buffer of 60 unsigned 32-bit integers of key
 *   schedule data.
 * - src: Source buffer of 32 bytes of key data.
 */
void pt_aes256_keyex(
  ...
  uint32_t tmp[60] = {
    ...
    src[19] | (src[18] << 8) | (src[17] << 16) | (src[16] << 24),
    src[23] | (src[22] << 8) | (src[21] << 16) | (src[20] << 24),
    src[27] | (src[26] << 8) | (src[25] << 16) | (src[24] << 24),
    src[31] | (src[30] << 8) | (src[29] << 16) | (src[28] << 24),
  };
...
/**
 * Implementation of AES-256 AddRoundKey transformation from FIPS-197.
 *
 * FIXME: should be merged with pt_aes{128,192}_add_round_key()
 *
 * Parameters:
 * - dst: Destination buffer for output block (16 bytes).
 * - src: Source buffer for input block (16 bytes).
 * - key_data: Key data for this round.
 */
static inline void pt_aes256_add_round_key(
  uint8_t dst[static restrict 16],
  const uint8_t src[static restrict 16],
  const uint32_t key_data[static restrict 4]
) {
...
/**
 * Encrypt a single block (16 bytes) of data using AES-256.
 *
 * Parameters:
 * - dst: destination buffer for output ciphertext block.
 * - src: source buffer for input plaintext block.
 * - key_data: Source buffer of 60 unsigned 32-bit integers containing
 *   key schedule data generated by pt_aes256_keyex().
 */
void pt_aes256_enc(
  uint8_t dst[static restrict 16],
  const uint8_t src[static restrict 16],
  ...
  // add initial round key
  pt_aes256_add_round_key(a, src, key_data);

  // first 13 rounds
  for (int i = 0; i < 13; i++) {
    pt_aes_enc_sub_and_shift(b, a);
    pt_aes_mix(a, b);
    pt_aes256_add_round_key(b, a, key_data + 4 * (i + 1));
    COPY(a, b, 16);
  }

  // final round
  pt_aes_enc_sub_and_shift(b, a);
  pt_aes256_add_round_key(dst, b, key_data + 56);
}

/**
 * Decrypt a single block (16 bytes) of data using AES-256.
 *
 * Parameters:
 * - dst: destination buffer for output plaintext block.
 * - src: source buffer for input ciphertext block.
...
void pt_aes256_dec(
  ...
  // add initial round key
  pt_aes256_add_round_key(a, src, key_data + 56);

  // first 13 rounds
  for (int i = 0; i < 13; i++) {
    pt_aes_dec_shift_and_sub(b, a);
    pt_aes256_add_round_key(a, b, key_data + 52 - 4 * i);
    pt_aes_inv_mix(b, a);
    COPY(a, b, 16);
  }
